			...
			foreach ($gbook as $roomres) {
				...
				foreach ($included_meals as $meal_enum => $meal_name) {
					if ($raw_type == 'arriving') {
						...
						$room_meals[] = substr($meal_name, 0, 1);
				...
				array_push($this->rows, array(
					...
					array(
						...
						'callback' => function ($val) use ($room_meals) {
							$meals_str = '';
							if ($room_meals) {
								$meals_str = ' <span class="vbo-wider-badges-wrap vbo-hidein-print"><span class="badge badge-info">' . implode(', ', $room_meals) . '</span></span><span class="vbo-showin-print">(' . implode(', ', $room_meals) . ')</span>';
							}
							return $val . $meals_str;
						},
						'export_callback' => function ($val) use ($room_meals) {
							return $val . ($room_meals ? ' (' . implode(', ', $room_meals) . ')' : '');
						},
						'value' => ucwords($rplan)
					),
					array(
